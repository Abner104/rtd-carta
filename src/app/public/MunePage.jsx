...
export default function MenuPage() {
  ...
  useSEO({
    title: settings?.business_name ? `${settings.business_name} · Carta Digital` : "Carta Digital",
    description: settings?.description || "Explorá nuestra carta de tragos y cocktails.",
    image: settings?.logo_url || undefined,
    url: window.location.href,
  }); // controlado por toggle, no por settings
  ...
  if (!loaded) return <SkeletonMenu bg={loaderColors.bg} />;
    ...
    <main className="min-h-screen transition-colors duration-300" style={{ backgroundColor: bg, color: text }}>
      ...
      {settings?.whatsapp && (
        <motion.a
          ...
          whileTap={{ scale: 0.95 }}
          className="fixed bottom-6 right-6 z-40 flex h-14 w-14 items-center justify-center rounded-full shadow-2xl"
